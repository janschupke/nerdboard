# PRP: Refactor Federal Funds Rate Tile to Unified Data Architecture

## Feature Overview

Refactor the Federal Funds Rate tile to use the new unified data fetching architecture, ensuring it uses `fetchAndMap` for API retrieval and is fully type-safe, testable, and consistent with the new structure.

## User Stories

- As a user, I want to see up-to-date US Federal Funds Rate data in the dashboard.
- As a developer, I want the Federal Funds Rate tile to use the centralized dataFetcher and dataMapper for all data access.

## Technical Requirements

- Update the Federal Funds Rate tile’s data hook to use `fetchAndMap` from `dataFetcher`.
...
- See `API_RESEARCH.md` Federal Funds Rate section.
- See `src/components/tile-implementations/federal-funds-rate/` for current implementation.
