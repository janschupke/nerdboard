...
export const FederalFundsRateTile = React.memo<FederalFundsRateTileProps>(({ size, config, ...rest }) => {
  ...
  if (configError) {
    content = (
      <div className="text-error-600 p-2">
        <span className="font-semibold">Tile Error:</span> Invalid or missing config for FederalFundsRateTile.
      </div>
    );
  } else if (loading) {
