// Grid system configuration - single source of truth for all grid and tile dimensions
...
export function getGridCellDimensions(rect: DOMRect) {
  return {
    width: rect.width / GRID_CONFIG.columns,
    height: rect.height / GRID_CONFIG.rows,
  };
}

export function getTileDimensions(tileSize: 'small' | 'medium' | 'large' = 'medium') {
  return getTileSpan(tileSize);
}
